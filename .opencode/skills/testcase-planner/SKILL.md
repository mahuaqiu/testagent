...
description: "代码库分析与生成计划。接收结构化测试步骤，搜索项目代码库中已有的 Pages/Services/Steps/fixtures，判断哪些可复用、哪些需新建，输出详细的代码生成计划供 testcase-coder 执行。不生成代码。"
---
...
已有 Pages/Services:
  - LoginPage (web/pages/login_page.py)
    ...
    - 定位器: loc_username, loc_password, loc_submit_btn
```
...
读取全局 `conftest.py` 和对应端的 `conftest.py`，记录所有 fixture 名称、scope、返回类型。

#### 3.4 扫描公共库

读取 `common/__init__.py` 和各模块，记录可用的断言函数、数据工厂方法、工具函数。

#### 3.5 扫描已有测试用例

搜索对应端 `tests/` 目录下所有 `test_*.py` 文件，记录文件名、测试类名、已有方法名（避免命名冲突）。
...
- **已有类但缺少方法** → 标记 "扩展: 在 LoginPage 中新增 fill_captcha()"
- **需要新建类** → 标记 "新建: HomePage (web/pages/home_page.py)"

#### 4.2 Steps 匹配

对于前置条件和重复出现的操作序列，判断：
- **已有 Steps 覆盖** → 标记 "复用: login_steps.login_as()"
- **已有 Steps 但缺少方法** → 标记 "扩展"
- **需要新建 Steps** → 标记 "新建: OrderSteps"
- **多条用例共享相同的多步骤前置操作** → 必须封装为 Steps

#### 4.3 Fixture 匹配

确认用例需要的 fixture 是否已存在，不存在则标记需要在 conftest.py 中新增。
...
- 文件: web/pages/home_page.py
  - 类名: HomePage，继承: BasePage
  - 定位器: loc_welcome_text, loc_user_menu
  - 方法: should_show_username(name), open_user_menu()

#### 2.2 新建 Steps（如有）
- 文件: web/steps/order_steps.py
  - 类名: OrderSteps
  - 组合: ProductPage, CartPage, CheckoutPage
  - 方法: add_product_to_cart(name), checkout(address)
  - 注册 fixture: order_steps → web/conftest.py

#### 2.3 扩展已有文件（如有）
- 文件: web/pages/login_page.py
  - 新增方法: fill_captcha(code: str)

### 3. 需要修改的 conftest.py（如有）
- web/conftest.py: 新增 fixture order_steps

### 4. 测试用例生成计划

#### 4.1 文件: web/tests/test_home.py（新建）
- 测试类: TestHome
- 用例:
  | 方法名 | 对应 TC | fixture | 依赖 Page/Steps |
  |--------|---------|---------|-----------------|
...
1. **精确到方法级别**：每个需要新建/扩展的方法都列出签名和用途
