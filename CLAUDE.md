...
检测设备锁屏状态并自动解锁。支持两种场景：
- **无密码设备**：唤醒屏幕 + 滑动解锁（不传密码参数）
- **有密码设备**：唤醒屏幕 + 滑动 + 输入密码（传密码参数）
...
通过 `PostMessage(WM_CLOSE)` 一次关闭**所有**匹配的窗口/弹窗。适合关闭系统对话框（如 `#32770`「声音」设置窗）等场景。
...
**行为说明**：
- 一次关闭所有匹配的可见窗口（不止 Z 序最前的第一个）
- 发送 `WM_CLOSE` 后每 0.3 秒重新枚举一次（最长 2 秒），期间新出现的匹配窗口（如连环弹窗）也会补发关闭请求
- 一个匹配窗口都没有（含已全部关闭后重复调用）→ `SUCCESS`（幂等，不报错）
- 所有匹配窗口销毁或隐藏 → `SUCCESS`
- 超时后仍有匹配窗口可见（如弹出「是否保存」）→ `FAILED`
