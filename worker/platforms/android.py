...
class AndroidPlatformManager(PlatformManager):
    ...
    def create_session(self, device_id: Optional[str] = None, options: Optional[Dict] = None) -> Session:
        ...
        logger.info(f"Android session created: {session_id} (device={device_id})")
    ...
    def close_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Android session closed: {session_id}")
            return True
    ...
    def _action_ocr_input(self, driver, action: Action) -> ActionResult:
        ...
        # 输入文本
        if action.value:
            driver.press_keycode(67)  # 清除
            driver.find_element(AppiumBy.CLASS_NAME, "android.widget.EditText").send_keys(action.value)
    ...
    def _action_press(self, driver, action: Action) -> ActionResult:
        ...
        key_name = action.value.upper() if action.value else ""
        key_code = key_map.get(key_name)

        if key_code:
            driver.press_keycode(key_code)
        elif action.value and action.value.isdigit():
            driver.press_keycode(int(action.value))
        else:
            return ActionResult(
                index=0,
                action_type="press",
                status=ActionStatus.FAILED,
                error=f"Unknown key: {action.value}",
            )
